...
export async function cashFlowForecastReport(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Opening cash = sum of BANK+CASH role account base balances as-of today
    // Use the same base-amount approach as loadAccountBalances in financialStatementsController.
    ...
    // ASSET accounts (BANK/CASH are ASSET-typed): balance = Σ(baseDebit) − Σ(baseCredit)
    let openingCashDecimal = new Prisma.Decimal(0);
    for (const a of cashBankAccounts) {
      const sumDebit = a.journalLines.reduce((s, l) => s.plus(l.baseDebit), new Prisma.Decimal(0));
      const sumCredit = a.journalLines.reduce((s, l) => s.plus(l.baseCredit), new Prisma.Decimal(0));
      openingCashDecimal = openingCashDecimal.plus(sumDebit.minus(sumCredit));
    }
    const openingCash = openingCashDecimal.toFixed(4);
