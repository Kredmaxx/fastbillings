  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (currentBalanceNum > 0) {
        if (!paymentMode) {
          throw new Error('PAYMENT_MODE_BANK_NOT_FOUND');
        }
      ...
      case 'SUPPLIER_PAYMENT': {
        if (transaction.relatedId) {
          const payment = await prisma.supplierPayment.findUnique({
            where: { id: transaction.relatedId },
            include: {
              purchase: { select: { purchaseId: true, totalAmount: true } },
              supplier: {
                select: {
                  id: true,
                  firstName: true,
          ...
          if (payment && payment.supplier) {
            const supplierProfileImage = payment.supplier.profileImage
              ? `${baseUrl}/${payment.supplier.profileImage}`
              : '';
            relatedData = {
              ...payment,
              supplierId: {
