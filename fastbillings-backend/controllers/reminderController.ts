...
const cronModule: { runReminderCron: () => Promise<unknown> } = require('../invoiceReminderCron');
...
export async function createReminder(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (type === 'automatic' || type === 'automatic_Purchase') {
      if (remindDays !== undefined) data.remindDays = remindDays;
...
export async function updateReminder(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (updateBody.remindDays !== undefined) data.remindDays = updateBody.remindDays as number;
...
export async function triggerReminderCron(_req: Request, res: Response): Promise<void> {
  try {
    console.log('Manual trigger of invoice reminder cron requested');

    // Run the cron job
    await cronModule.runReminderCron();
