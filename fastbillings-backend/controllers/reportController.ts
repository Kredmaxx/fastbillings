...
export async function getInventoryReport(req: Request, res: Response): Promise<void> {
  try {
    ...
    const totalsInventoryMap: Record<string, number> = {};
    inventoriesForTotals.forEach((inv) => {
      totalsInventoryMap[inv.productId] = inv.quantity;
    });
    ...
    allProductsForTotals.forEach((product) => {
      const stockQty = totalsInventoryMap[product.id] ?? 0;
      const sellingPrice = asNumber(product.selling_price, 0);
      totalValues += stockQty * sellingPrice;
      if (stockQty > 0 && stockQty <= product.alert_quantity) lowStockItemsCount++;
      if (stockQty === 0) outOfStockItemsCount++;
    });

    const totalProducts = allProductsForTotals.length;
    ...
    const inventoryMap: Record<string, { quantity: number }> = {};
    inventories.forEach((inv) => {
      inventoryMap[inv.productId] = { quantity: inv.quantity };
    });

    const data = products.map((product) => {
      const inventoryInfo = inventoryMap[product.id] ?? null;
      ...
      return {
        ...
        stock: inventoryInfo ? stockQty : 0,
      };
    });
...
export async function getBestSellerReport(req: Request, res: Response): Promise<void> {
  try {
    ...
    const inventoryMap: Record<string, (typeof inventories)[number]> = {};
    inventories.forEach((inv) => {
    ...
    const data = products.map((product) => {
      const inventoryInfo = inventoryMap[product.id] ?? null;
      const history = toHistoryArray(inventoryInfo?.inventory_history);
      ...
      const soldQtyCurrent = currentHistory.reduce(
        (sum, h) => sum + Math.abs(asNumber(h.adjustment, 0)),
        0,
      );
      ...
      const sellingPrice = asNumber(product.selling_price, 0);
      const soldAmountCurrent = soldQtyCurrent * sellingPrice;
      const soldAmountPrevious = soldQtyPrevious * sellingPrice;

      totalSalesCurrent += soldAmountCurrent;
      totalSalesPrevious += soldAmountPrevious;

      const lastEntry = currentHistory.length
    ...
    // Average sales
    const avgSalesCurrent = products.length > 0 ? totalSalesCurrent / products.length : 0;
    const avgSalesPrevious = products.length > 0 ? totalSalesPrevious / products.length : 0;
