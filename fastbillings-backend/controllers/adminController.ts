...
export async function getCountries(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (search) {
      ...
      const countries = await prisma.country.findMany({ where, select });
      res.json(countries);
    } else {
      const countries = await prisma.country.findMany({ where, select, take: 10 });
      res.json(countries);
    }
  } catch (err) {
    res.status(500).json({
...
export async function getStates(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (search) {
      where.name = { contains: search, mode: 'insensitive' };
      const states = await prisma.state.findMany({ where, select });
      res.json(states);
    } else {
      const states = await prisma.state.findMany({ where, select, take: 10 });
      res.json(states);
    }
  } catch (err) {
    res.status(500).json({
...
export async function getCities(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (search) {
      ...
      const cities = await prisma.city.findMany({ where, select, take: 10 });
      res.json(cities);
    }
  } catch (err) {
    res.status(500).json({
...
export async function updateProfile(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (body.gender !== undefined)
      updateData.gender = body.gender as Prisma.UserUpdateInput['gender'];
