...
export async function createExpense(
  ...
  try {
    ...
    /* ===========================
       ...
       configured rate are rejected as a clean 422 before balance mutations.
    =========================== */
    ...
    const savedExpense = await prisma.$transaction(async (tx) => {
      ...
      if (sourceType === 'BANK') {
        ...
        const balanceAfter = Number(
          (balanceBefore - expenseAmount).toFixed(2),
        );
    ...
    const formattedExpenses = expenses.map((exp) => {
      ...
      return {
        ...
        supplier: exp.supplier
          ? { id: exp.supplier.id, name: exp.supplier.supplier_name }
          : null,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        supplier: expense.supplier
          ? { id: expense.supplier.id, name: expense.supplier.supplier_name }
          : null,
    ...
    const updatedExpense = await prisma.$transaction(async (tx) => {
      ...
      if (oldSourceType !== sourceType) {
        if (oldSourceType === 'BANK' && oldBankId) {
          ...
          if (bank) {
            const newBalance = Number(
              (Number(bank.currentBalance ?? 0) + oldAmount).toFixed(2),
            );
            await tx.bankDetail.update({
              where: { id: bank.id },
          ...
          if (pettyCash) {
            const newBalance = Number(
              (Number(pettyCash.currentBalance ?? 0) + oldAmount).toFixed(2),
            );
            await tx.pettyCash.update({
              where: { id: pettyCash.id },
              data: { currentBalance: toDecimal(newBalance) },
        ...
        if (sourceType === 'BANK') {
          ...
          const balanceBefore = Number(bank.currentBalance ?? 0);
          const balanceAfter = Number(
            (balanceBefore - asNumber(amount, 0)).toFixed(2),
          );
      ...
      /* =====================================
         GL POSTING: reverse old entry, re-post with updated amounts
      ===================================== */
      {
...
export async function getRecurringExpenses(req: Request, res: Response): Promise<void> {
  try {
    ...
    const data = rows.map((exp) => ({
      ...
      referenceNo: exp.referenceNo,
      amount: exp.amount,
      supplier: exp.supplier ? { id: exp.supplier.id, name: exp.supplier.supplier_name } : null,
