...
export async function createRole(req: Request, res: Response): Promise<void> {
  try {
    const { roleName, status = true } = req.body as {
      roleName?: string;
      status?: boolean;
    };
...
export async function getRoles(req: Request, res: Response): Promise<void> {
  try {
    ...
    const where: Prisma.RoleWhereInput = { deletedAt: null, tenantId };
...
export async function listUsersByRole(req: Request, res: Response): Promise<void> {
  try {
    ...
    const where: Prisma.UserWhereInput = { id: { in: memberships.map((m) => m.userId) } };
...
export async function updateRole(req: Request, res: Response): Promise<void> {
  try {
    ...
    const tenantId = requireTenantId(req);
    const { roleName, status } = req.body as {
      roleName?: string;
      status?: boolean;
    };
...
export async function deleteRole(req: Request, res: Response): Promise<void> {
  try {
    ...
    const userCount = await prisma.tenantMembership.count({ where: { roleId: id, tenantId } });
    if (userCount > 0) {
      res.status(409).json({
        success: false,
