...
exports.ssoExchange = async (req, res) => {
  ...
  if (!secret) {
    return res.status(503).json({
      success: false,
      message: 'SSO not configured on this FastBillings instance.',
    });
  }
...
exports.upsertCustomer = async (req, res) => {
  ...
  if (!ownerId) {
    return res.status(409).json({
      success: false,
      message: 'No admin user configured on FastBillings yet — register an admin first.',
    });
  }
  ...
  try {
    ...
    if (err && err.code === 'P2002') {
      return res.status(409).json({
        success: false,
        message: 'A FastBillings customer with this email already exists outside the integration.',
