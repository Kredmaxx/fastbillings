...
import { tenantEntityScope, requireTenantId, requireUserId, UnauthorizedError } from '../lib/tenantScope';
...
export async function createMinimalCustomer(req: Request, res: Response): Promise<void> {
  try {
    const userId = requireUserId(req);
    const tenantId = requireTenantId(req);
    const { name, email, phone, currencyCode: rawCurrencyCode } = req.body as {
      name?: string;
      email?: string;
      phone?: string;
      currencyCode?: string;
...
export async function getStatement(req: Request, res: Response): Promise<void> {
  try {
    ...
    const allPayments = await prisma.invoicePayment.findMany({
      where: { invoice: { billTo: id, tenantId, isDeleted: false }, received_on: { lte: toDate } },
      select: {
        id: true,
        invoiceId: true,
        amount: true,
