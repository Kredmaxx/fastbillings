  ...
  try {
    ...
    const [recInvoices, dueInvoices, recExpenses] = await Promise.all([
      prisma.invoice.findMany({
        ...
        select: { id: true, invoiceNumber: true, TotalAmount: true, nextRecurringDate: true, repeatEvery: true, customer: { select: { name: true } } },
      ...
      prisma.invoice.findMany({
        ...
        select: { id: true, invoiceNumber: true, TotalAmount: true, dueDate: true, payments: { select: { amount: true } }, customer: { select: { name: true } } },
        orderBy: { dueDate: 'asc' },
      }),
      prisma.expense.findMany({
        where: { userId, isDeleted: false, isRecurring: true, nextRecurringDate: { gte: nextStart, lte: nextEnd } },
    ...
    res.json({
      ...
      data: {
        ...
        upcomingRecurringInvoices: recInvoices.map((i) => ({
          id: i.id,
          label: i.invoiceNumber ?? i.customer?.name ?? 'Recurring invoice',
          amount: num(i.TotalAmount),
          date: i.nextRecurringDate,
          frequency: i.repeatEvery,
        })),
        upcomingDueInvoices: dueInvoices
          .map((i) => ({ id: i.id, label: i.invoiceNumber ?? i.customer?.name ?? 'Invoice', amount: outstandingOf(i), date: i.dueDate }))
          .filter((x) => x.amount > 0),
