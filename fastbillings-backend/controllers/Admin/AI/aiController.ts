...
export async function generateFollowup(req: Request, res: Response): Promise<void> {
  try {
    ...
    const customerName =
      invoice.customer?.name || invoice.billToCustomer?.name || 'Customer';
    const customerEmail =
      invoice.customer?.email || invoice.billToCustomer?.email || null;
...
export async function sendFollowup(req: Request, res: Response): Promise<void> {
  try {
    ...
    await sendMail({
      from: `"${process.env.SMTP_FROM_NAME || 'Accounts'}" <${process.env.SMTP_EMAIL}>`,
      to,
      subject,
      html: body,
      tenantId,
