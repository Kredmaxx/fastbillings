...
import { requireUserId, tenantOrUserFilter } from '../lib/tenantScope';

/**
 * Daily AI call quota (Cluster H, slice H.4).
 *
 * Counts workspace `AiUsageLog` rows over a rolling 24h window. When the
...
export async function aiRateLimit(
  ...
  try {
    ...
    const count = await prisma.aiUsageLog.count({
      where: { ...ownership, createdAt: { gte: windowStart } },
    });
