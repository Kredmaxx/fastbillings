...
vi.mock('@prisma/client', async () => {
  ...
  const PrismaClient = vi.fn(() => ({
    ...
    user: {
      updateMany: vi.fn(async (args: { where: { user_type: number; roleId: null }; data: { roleId: string } }) => {
        let count = 0;
        for (const u of store.users) {
          if (u.user_type === args.where.user_type && u.roleId === null) {
...
describe('DEFAULT_ROLE_BY_USER_TYPE', () => {
  it('contains exactly the 6 expected roles', () => {
    expect(DEFAULT_ROLE_BY_USER_TYPE[0]).toBe('Super Admin');
    ...
    expect(Object.keys(DEFAULT_ROLE_BY_USER_TYPE)).toHaveLength(6);
  });
});
...
describe('seedRoles', () => {
  ...
  it('creates exactly 6 roles on first run', async () => {
  ...
  it('assigns correct role id for each user_type', async () => {
    ...
    expect(store.users.find((u) => u.id === 'u1')!.roleId).toBe(result.roleIds[1]);
