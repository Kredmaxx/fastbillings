...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ( { moduleSlug, action}) => {
    const { isAuthenticated, isLoading, user, token, activeTenantId, tenants } = useSelector((state: RootState) => state.auth);
    ...
    if (hasFullTenantAccess(user?.user_type, membership?.role)) {
        return <Outlet />;
    }
