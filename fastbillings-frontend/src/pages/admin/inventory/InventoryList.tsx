...
const InventoryList: React.FC = () => {
    ...
    const handleHistory = async (inventory: InventoryData) => {
        try {
            const response = await axios.get(`${Constants.FETCH_INVENTORY_HISTORY_URL}/${inventory.id}`, {
                headers: { 'Authorization': `Bearer ${token}` },
            })
                    ...
                    <TableRow
                        ...
                        columns={[
                            ...
                            < div className="flex gap-2" >
                                {/* History */}
                                <span
                                    onClick={(e) => { e.stopPropagation(); handleHistory(inventory); }}
                        ...
                        onRowClick={(item) => handleHistory(item)}
                    />
                ))}
                {
