...
const CreateInvoice: React.FC = () => {
    ...
    // Recompute a single line's taxes + totalTax based on its appliedTaxRateIds.
    ...
    const updateLineTaxes = (rowId: string, appliedTaxRateIds: string[]) => {
        setInvoiceFormData((prev) => ({
            ...
            items: prev.items.map((item) =>
                item.id === rowId ? recomputeLineTaxes(item, appliedTaxRateIds) : item,
            ),
        }));
    };
    ...
    const handleSuggestTaxesForLine = async (rowId: string) => {
        ...
        try {
            ...
            // Merge suggestions into the library so picker can show them too.
    ...
    const { subTotal, totalTax, totalDiscount, grandTotal } = useMemo(() => {
        ...
        const grandTotalInteger = Math.floor(grand_total);
    ...
    const handleInLineItemChange = (product: ProductItem, rowId: string) => {
        ...
        // If user has picked TaxRates for this line, recompute the new per-line tax breakdown.
        ...
        <div className="md:p-4 bg-white-50 min-h-screen border border-gray-200 rounded">
            <form>
                <div className="max-w-7xl mx-auto space-y-4">
                    ...
                    <div className="mb-4">
                        ...
                        <select
                            ...
                            className="mt-1 text-gray-700 p-2 focus:outline-none focus:ring-1 focus:ring-purple-600 focus:border-purple-600 border border-gray-200 rounded-md"
                        >
                    ...
                    <div className="w-full">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full">
                            <div className="w-full">
                                <label htmlFor="invoiceNumber" className="block text-sm font-medium text-gray-900">
                                    Invoice Number
                                </label>
                                ...
                                {formErrors?.invoiceNumber && <span className="text-red-500 text-sm">{formErrors.invoiceNumber}</span>}
                            </div>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-2 text-xs">
                                                        ...
                                                        <span className="ml-auto flex items-center gap-3">
                                                            <TaxPicker
                                                                ...
                                                                selectedIds={(item as ProductItem).appliedTaxRateIds ?? (item.taxes ?? []).map((t) => t.taxRateId)}
                ...
                <div className="flex justify-end mt-4 gap-3">
                    ...
                    <button
                        type="button"
                        className="px-4 py-2 text-sm font-medium text-white bg-purple-600 border border-transparent rounded-md shadow-sm hover:bg-gray-950 focus:outline-none flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer"
