...
// Landing page that completes the whatsappcrm → FastBillings SSO handshake.
// 1. Reads the short-lived JWT from ?token=
// 2. Exchanges it for a FastBillings session token via the backend
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const { token: sessionToken, user, tenants = [], activeTenantId } = res.data || {};
                if (!sessionToken || !user) {
                    throw new Error("Malformed SSO response.");
                }

                Cookies.set("authToken", sessionToken, { secure: true, sameSite: "Strict", expires: 7 });
