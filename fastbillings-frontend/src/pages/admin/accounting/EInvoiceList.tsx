...
const StatusBadge = ({ status }: { status: EInvoiceStatus }) => {
    const palette: Record<EInvoiceStatus, string> = {
    ...
    return (
        <span className={`inline-flex items-center px-2 py-1 rounded-sm text-xs font-medium ${palette[status]}`}>
            {status}
        </span>
    );
};
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-800">E-Invoices (IRN)</h1>
