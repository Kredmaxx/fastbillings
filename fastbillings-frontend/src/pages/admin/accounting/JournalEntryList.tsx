...
const JournalEntryList: React.FC = () => {
    ...
    const tableActions = [
        ...
        { label: "Delete", icon: <Trash2Icon size={14} />, onClick: (row: JournalEntryRow) => setDeleteItem(row) },
    ];
                ...
                <div className="fixed inset-0 bg-black/40 z-[100] flex items-center justify-center" onClick={() => setViewing(null)}>
                    <div className="bg-white rounded-md p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="text-right mt-4">
                            <button type="button" onClick={() => setViewing(null)} className="px-3 py-1 text-sm border rounded-md">Close</button>
                        </div>
                    </div>
                </div>
            )}
