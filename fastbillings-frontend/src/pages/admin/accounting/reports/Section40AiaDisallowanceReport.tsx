...
export default function Section40AiaDisallowanceReport() {
  ...
  async function load() {
    ...
    try {
      const r = await axios.get(
        `${Constants.GET_SECTION_40A_IA_DISALLOWANCE_URL}?fy=${encodeURIComponent(fy)}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      setData(r.data?.data ?? null);
    } catch {
      setError('Failed to load §40(a)(ia) disallowance worksheet');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto bg-white space-y-4">
      ...
      <div className="flex items-center justify-between print:hidden">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Resident purchases with TDS not deducted or not mapped to a challan (30% of taxable).
            Non-residents →{' '}
            <Link
      ...
      {data && (
        <ReportPrintShell
          printId="section-40a-ia-print-root"
          title="§40(a)(ia) TDS — books screen"
