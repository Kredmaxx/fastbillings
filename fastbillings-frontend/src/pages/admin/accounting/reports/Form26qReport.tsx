...
export default function Form26qReport() {
  const token = useSelector((s: RootState) => s.auth.token);
  const { formatDate } = useDateFormatter();
  const [quarter, setQuarter] = useState(currentQuarterLabel());
  const [data, setData] = useState<Form26qData | null>(null);
  ...
  async function load() {
    ...
    try {
      const r = await axios.get(
        `${Constants.GET_FORM_26Q_URL}?quarter=${encodeURIComponent(quarter)}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      setData(r.data?.data ?? null);
    } catch {
      setError('Failed to load Form 26Q worksheet');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto bg-white space-y-4">
      ...
      <div className="flex items-center justify-between print:hidden">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Quarterly TDS annexure for resident deductees (non-residents → Form 27Q).
          </p>
        </div>
        <button
          type="button"
      ...
      {data && (
        <ReportPrintShell
          printId="form-26q-print-root"
          title="Form 26Q — Statement of TDS under section 200(3)"
