...
const emptyForm = (): IForm => ({ code: "", name: "", status: "active" });
...
const Projects: React.FC = () => {
    ...
    const tableActions = [
        { label: "Edit", icon: <Edit size={14} />, onClick: (item: IProject) => openEdit(item) },
        { label: "Delete", icon: <Trash2 size={14} />, onClick: (item: IProject) => handleDeleteClick(item) },
    ];
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                ...
                >
                    <CirclePlusIcon size={14} /> New Project
                </button>
            </div>
            ...
            <Table headers={tableHeaders}>
                    ...
                    items.map((item, index) => (
                        <TableRow
                            ...
                            columns={[
                                ...
                                item.name,
                                <span className={`px-2 py-0.5 text-xs font-medium rounded-full capitalize ${statusBadgeClass(item.status)}`}>{item.status}</span>,
                            ]}
                ...
                {!isLoading && items.length === 0 && (
                    <tr><td className="text-center py-4 font-semibold text-gray-500" colSpan={5}>No Projects Found</td></tr>
                )}
                {isLoading && (
