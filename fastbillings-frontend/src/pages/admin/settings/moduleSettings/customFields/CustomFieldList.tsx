...
const CustomFieldList: React.FC<CustomFieldListProps> = ({ moduleSlug }) => {
    ...
    const tableHeaders = ['#', 'Label', 'Slug', 'Type', 'Mandatory', 'Show in Table', 'Actions'];
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
                <h4 className="text-xl font-semibold text-gray-800 font-sans">
                    Custom Fields for {currentModule?.moduleName || moduleSlug}
                </h4>
                <button
                    onClick={handleCreateClick}
                    disabled={isLoading}
                    className="bg-purple-600 hover:bg-purple-700 text-white px-3 py-1.5 rounded-md shadow flex items-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer text-sm font-medium"
                >
                    <CirclePlusIcon size={16} /> New Custom Field
                </button>
            </div>
            ...
            <Table headers={tableHeaders}>
                {!isLoading && customFields.map((field: any, index: number) => {
                    ...
                    return (
                        <TableRow
                            ...
                            index={from + index}
                            columns={[
                                <span className="font-medium text-gray-900">{field.labelName}</span>,
                                <span className="text-gray-500 font-mono text-sm">{field.fieldSlug}</span>,
                                <span className="bg-purple-50 text-purple-700 border border-purple-100 text-xs px-2.5 py-1 rounded-md font-medium">{typeName}</span>,
                ...
                {!isLoading && customFields.length === 0 && (
                    <tr>
                        <td colSpan={tableHeaders.length} className="text-center text-gray-500 py-6 font-medium">
