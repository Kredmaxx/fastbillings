...
import { CirclePlusIcon, Edit, RefreshCcwIcon, Trash2Icon } from "lucide-react";
...
const PurchaseOrderList: FC = () => {
    ...
    const [isConvertConfirmationModalOpen, setIsConvertConfirmationModalOpen] = useState<boolean>(false);
    const [itemToConvert, setItemToConvert] = useState<PurchaseOrder | null>(null);
    ...
    const getTableActions = (item: PurchaseOrder) => {
        const tableActions = [
            {
                label: 'Convert to Purchase',
                icon: <RefreshCcwIcon size={14} />,
                onClick: (item: PurchaseOrder) => { handleConvertToPurchaseClick(item) },
                hide: (item.convertedToPurchase)
            },
            {
                label: 'Edit',
                icon: <Edit size={14} />,
        ...
        return tableActions.filter((action) => {
            ...
            //if action is in restrictedActions, hide if status matches
            if (restrictedActions.includes(actionLabel)) {
                const permissionAction: PermissionAction =
                    actionLabel === 'delete' ? 'delete' : 'edit';
                return hasPermission(permissions, 'purchase-order', permissionAction);
            }
            //else, return true
            return true;
        });
    ...
    const handleConvertToPurchaseClick = (item: PurchaseOrder) => {
        setItemToConvert(item);
        setIsConvertConfirmationModalOpen(true);
    }

    const handleConvertToPurchase = async () => {
        try {
            setIsConverting(true);
            await axios.post(Constants.CONVERT_PO_TO_PURCHASE_URL, { purchaseOrderId: itemToConvert?.id },
                {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
            toast.success('Purchase order converted to purchase successfully.');
            fetchPurchaseOrders(search, limit, page);
            setIsConvertConfirmationModalOpen(false);
    ...
    return (
        <div className="space-y-4">
            ...
            <Table headers={tableHeaders}>
                {!isPageLoading && purchaseOrders && purchaseOrders.map((purchaseOrder, index) => (
                    <TableRow
                        ...
                        onRowClick={(item) => navigate(`/admin/purchase-orders/edit/${item.id}`)}
                    />
                ))}
            ...
            <DeleteConfirmationModal
                isOpen={isConvertConfirmationModalOpen}
                onClose={() => setIsConvertConfirmationModalOpen(false)}
                onConfirm={handleConvertToPurchase}
                isDeleting={isConverting}
                title="Confirm Conversion"
                message={`Are you sure you want to convert the purchase order ${itemToConvert?.purchaseOrderId} to a purchase? This action cannot be undone.`}
            />
        </div>
    );
}
