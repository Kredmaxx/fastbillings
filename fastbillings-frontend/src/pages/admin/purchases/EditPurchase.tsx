...
const EditPurchase: React.FC = () => {
    ...
    useEffect(() => {
        if (!purchaseFormData.tdsSection || purchaseFormData.tdsRateId || tdsRates.length === 0) return;
        const match = tdsRates.find(
            (r) => r.section.toUpperCase() === String(purchaseFormData.tdsSection).toUpperCase(),
        );
        if (match) {
            setPurchaseFormData((prev) => ({ ...prev, tdsRateId: match.id }));
        }
    }, [tdsRates, purchaseFormData.tdsSection, purchaseFormData.tdsRateId]);
    ...
    const handleSupplierChange = async (user: OptionType) => {
        ...
        try {
            ...
            setPurchaseFormData(prev => ({ ...prev, billTo: '' }));
        } finally {
            setIsFetching(false);
        }
    };
        ...
        try {
            ...
            if (data) {
                ...
                setPurchaseFormData(prev => ({
                    ...
                    tdsRatePercent: data.tdsRatePercent != null ? Number(data.tdsRatePercent) : null,
                    tdsAmount: data.tdsAmount != null ? Number(data.tdsAmount) : null,
            ...
            <form onSubmit={savePurchaseOrder}>
                <div className="max-w-7xl mx-auto space-y-4">
                    ...
                    <div className="flex justify-between items-center mb-2">
                        <h1 className="text-2xl font-bold text-gray-950 ">Edit Purchase</h1>
                    ...
                    <div className="w-full">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 w-full">
                            <div className="w-full">
                                <label htmlFor="po-id" className="block text-sm font-medium text-gray-700 ">
                                    Order ID
                                </label>
                                <input
                                    type="text"
                    ...
                    <div>
                        ...
                        {activeInfoTab === 'notes' && (
                            <div>
                                ...
                                <textarea value={purchaseFormData.notes} onChange={(e) => handleFormChange('notes', e.target.value)} rows={4} placeholder="Enter Notes" className="border border-gray-300 rounded-md px-4 py-2 w-full  text-gray-950  focus:outline-none focus:ring-1 focus:ring-purple-600"></textarea>
                            </div>
                        )}
                        {activeInfoTab === 'termsAndCondition' && (
                            <div>
                                <label className="block text-sm font-medium text-gray-700 ">Terms & Conditions</label>
                                <textarea value={purchaseFormData.termsAndCondition} onChange={(e) => handleFormChange('termsAndCondition', e.target.value)} rows={4} placeholder="Enter Terms & Conditions" className="border border-gray-300 rounded-md px-4 py-2 w-full  text-gray-950  focus:outline-none focus:ring-1 focus:ring-purple-600"></textarea>
                            </div>
                        )}
                        {activeInfoTab === 'bank' && (
                            <div>
                    ...
                    <div className="bg-white  p-4 rounded-lg border border-gray-200  space-y-3">
                        ...
                        {purchaseFormData.sign_type !== 'none' && (purchaseFormData.sign_type === 'digitalSignature' ? (
                            ...
                            <div>
                                ...
                                <input name='signatureName' type="text" value={purchaseFormData.signatureName} onChange={e => handleFormChange('signatureName', e.target.value)} placeholder="Enter Signature Name" className="border border-gray-300 rounded-md px-4 py-2 w-full text-gray-950 focus:outline-none focus:ring-1 focus:ring-purple-600" />
